  ...
  <main class="min-h-screen bg-slate-900 text-white">
    <section class="mx-auto max-w-4xl px-4 py-10">
      <header class="mb-8 text-center">
        ...
        <h1 class="text-4xl font-bold">点歌 KTV 系统</h1>
        <p class="mt-2 text-slate-300">在线搜歌、点歌、播歌</p>
      </header>

      <SearchBar
        v-model="keyword"
        :loading="loading"
        placeholder="请输入歌曲名，例如：演员"
        @submit="searchSongs"
      />
...
const playSong = async (path) => {
  try {
    ...
    if (!response.ok) {
      throw new Error("播放失败，请检查 MPV 及路径映射");
    }
  } catch (err) {
    error.value = err.message;
  }
